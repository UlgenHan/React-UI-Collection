...
export interface SidebarWithIconsProps {
  logo?: LogoProps;
...
export interface NavItemWithIcon extends NavItem {
  ...
  iconType?: 'home' | 'dashboard' | 'users' | 'settings' | 'analytics' | 'documents' | 'messages' | 'calendar';
}

const SidebarWithIcons: React.FC<SidebarWithIconsProps> = ({
  logo = { text: 'Logo' },
  navItems = [],
  bgColor = 'bg-white',
  className = ''
          ...
          <ul className="space-y-1 px-4">
            {navItems.map((item, index) => (
              <li key={index} className="relative group">
                ...
                <div className="absolute left-full top-0 ml-2 px-3 py-2 bg-gray-900 text-white text-sm rounded-lg opacity-0 group-hover:opacity-100 transition-all duration-200 pointer-events-none whitespace-nowrap z-50 transform translate-x-1 group-hover:translate-x-0">
                  {item.label}
